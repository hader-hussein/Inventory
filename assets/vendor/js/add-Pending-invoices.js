...
function updateRowNumbersee() {
    $('#my-table-invoices tr').each(function (index, row) {
        $(row).find('td:first').text(index + 0); // الترقيم يبدأ من 1
    });
}
//table-Perished
 // Add new row when plus button is clicked
 $('#my-table-invoices').on('click', '.add-invoices', function () {
    var newRowe = `
    <tr>
    <td></td>
           <td>  
